...
class ProductsControllerTest < ActionController::TestCase
  ...
  test 'Если только одно предложение, то оно по-умолчанию выделено' do
    flunk 'Невозможно удалить точки :\ '
  end

  test 'И наоборот. Если несколько предложений, то ни одно не выделено' do
    flunk 'Ну это легко протестировать, но позже'
  end

  test 'Если товар помещен в корзину, то вместо кнопки "Добавить в корзину" отображается кнопка "Товар в корзине"' do
    flunk 'Раньше был текст, обёрнутый в H4 и у него был id, сейчас он переехал к кнопке, вроде так стало даже более консистентнее (как у кнопки Добавить в корзину. Пересмотреть)'
  end

  test 'Если товар поместили в корзину, то вместо кнопки "Добавить в корзину" отображается кнопка "Перейти в корзину"'
    flunk "Аналогично предыдущему тесту"
  end
