    ...
    def self.egrgroup_ru_import
      ...
      CSV.foreach(path) do |row|
        ...
        if !row[1].blank? && !row[2].blank? && !row[3].blank? && !row[4].blank? && !row[5].blank? && !row[6].blank? && !row[7].blank?
          ...
          @parsed_json = PriceMate.search(row[1], row[3], false, false, false)
          ...
          if @parsed_json["result_prices"].present?
            ...
            manufacturer = BrandMate.find_or_create_canonical!(row[5])
            spare_catalog = SpareCatalog.find_or_create_by(name: row[6].mb_chars.upcase)
            spare_info = SpareInfo.find_or_create_by(catalog_number: row[1].gsub('-', ''), brand: manufacturer, spare_catalog: spare_catalog)
            spare_info.assign_attributes(
              content: row[7]
            )
