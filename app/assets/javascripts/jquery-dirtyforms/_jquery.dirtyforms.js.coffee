...
class _DirtyForms
  constructor: ->
    ...
    @highlight_dirty = (el) -> 
      $(el).closest('.form-group').addClass('dirty')
    ...
    #$(document).on 'ajax:beforeSend', @forms_selector, @rails

    $(document).on 'page:before-change', @rails
    #  alert 'a1'
  ...
  rails: (e) =>
    ...
    if (!op1 && !op2)
    #unless ( $(e.target).is(@forms_selector) ) && op1
      if @check_all_dirty()
