class Discourse
  ...
  def link_to_new
    "http://#{Rails.application.config_for('application/discourse')['host_port']}/new-topic?title=#{URI.encode(search_string)}&body=#{URI.encode('Удалите этот текст и напишите свой. После сохранения можете вернуться назад и обновить страницу, текст сразу отобразится на сайте. Не меняйте заголовок сообщения.')}&category=#{@category['id']}"
  end
